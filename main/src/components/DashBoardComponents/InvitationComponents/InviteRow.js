...
const InviteRow = (props) => {
    ...
    const acceptHandler = async() => {
        try {
            const response = await axios.post(process.env.REACT_APP_BASE_URL + "/api/v1/acceptInvite", {
    ...
    const rejectHandler = async() => {
        try {
            const response = await axios.post(process.env.REACT_APP_BASE_URL + "/api/v1/rejectInvite", {
              "email": props.user.data.email,
              "projectId":props.data["_id"],
            });
